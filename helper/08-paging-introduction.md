...
1. [Memory Protection](#memory-protection)
2. [Segmentation](#segmentation)
   1. [Virtual Memory](#virtual-memory)
   2. [Fragmentation](#fragmentation)
3. [Paging](#paging)
   1. [Hidden Fragmentation](#hidden-fragmentation)
   2. [Page Tables](#page-tables)
   3. [Multilevel Page Tables](#multilevel-page-tables)
4. [Paging on x86_64](#paging-on-x86_64)
   1. [Example Translation](#example-translation)
   2. [Page Table Format](#page-table-format)
   3. [The Translation Lookaside Buffer](#the-translation-lookaside-buffer)
5. [Implementation](#implementation)
   1. [Page Faults](#page-faults)
   2. [Accessing the Page Tables](#accessing-the-page-tables)
6. [Summary](#summary)
...
![Two virtual address spaces with address 0–150, one translated to 100–250, the other to 300–450](../assets/segmentation-same-program-twice.svg)
...
![Three virtual address spaces, but there is not enough continuous space for the third](../assets/segmentation-fragmentation.svg)
...
![Three virtual address spaces after defragmentation](../assets/segmentation-fragmentation-compacted.svg)
...
![With paging the third program instance can be split across many smaller physical areas](../assets/paging-fragmentation.svg)
...
![Three page tables, one for each program instance. For instance 1 the mapping is 0->100, 50->150, 100->200. For instance 2 it is 0->300, 50->350, 100->400. For instance 3 it is 0->250, 50->450, 100->500.](../assets/paging-page-tables.svg)
...
![Page 0 mapped to frame 0 and pages `1_000_000`–`1_000_150` mapped to frames 100–250](../assets/single-level-page-table.svg)
...
![Bits 0–12 are the page offset, bits 12–21 the level 1 index, bits 21–30 the level 2 index, bits 30–39 the level 3 index, and bits 39–48 the level 4 index](../assets/x86_64-table-indices-from-address.svg)
...
![The sign extension bits are all 0, the level 4 index is 1, the level 3 index is 0, the level 2 index is 511, the level 1 index is 127, and the page offset is 0x5ce](../assets/x86_64-page-table-translation-addresses.png)
...
- 512 _512_ 512 level 1 tables (512 entries for each level 2 table).
...
- The `huge page` flag allows to create pages of larger sizes by letting the entries of the level 2 or level 3 page tables directly point to a mapped frame. With this bit set, the page size increases by factor 512 to either 2MiB = 512 _4KiB for level 2 entries or even 1GiB = 512_ 2MiB for level 3 entries. The advantage of using larger pages is that fewer lines of the translation cache and fewer page tables are needed.
...
A 4-level page table makes the translation of virtual addresses expensive, because each translation requires 4 memory accesses. To improve performance, the x86\*64 architecture caches the last few translations in the so-called \_translation lookaside buffer\* (TLB). This allows to skip the translation when the translation is still cached.
...
["a minimal rust kernel"]: 02-minimal-rust-kernel.md#creating-a-bootimage
...
[double fault]: 06-double-faults.md
...
[`hlt_loop`]: 07-hardware-interrupts.md#the-hlt-instruction
...
![EXCEPTION: Page Fault, Accessed Address: VirtAddr(0xdeadbeaf), Error Code: CAUSED_BY_WRITE, InterruptStackFrame: {…}](../assets/qemu-page-fault.png)
...
![QEMU with output: "read worked, EXCEPTION: Page Fault, Accessed Address: VirtAddr(0x2031b2), Error Code: PROTECTION_VIOLATION | CAUSED_BY_WRITE, InterruptStackFrame: {…}"](../assets/qemu-page-fault-protection.png)
