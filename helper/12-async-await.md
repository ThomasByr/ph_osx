...
1. [Multitasking](#multitasking)
   1. [Preemptive Multitasking](#preemptive-multitasking)
      1. [Saving State](#saving-state)
      2. [Discussion](#discussion)
   2. [Cooperative Multitasking](#cooperative-multitasking)
      1. [Saving State](#saving-state-1)
      2. [Discussion](#discussion-1)
2. [Async/Await in Rust](#asyncawait-in-rust)
   1. [Futures](#futures)
      1. [Example](#example)
      2. [Futures in Rust](#futures-in-rust)
   2. [Working with Futures](#working-with-futures)
      1. [Waiting on Futures](#waiting-on-futures)
      2. [Future Combinators](#future-combinators)
         1. [Advantages](#advantages)
         2. [Drawbacks](#drawbacks)
   3. [The Async/Await Pattern](#the-asyncawait-pattern)
      1. [State Machine Transformation](#state-machine-transformation)
      2. [Saving State](#saving-state-2)
      3. [The Full State Machine Type](#the-full-state-machine-type)
   4. [Pinning](#pinning)
      1. [Self-Referential Structs](#self-referential-structs)
      2. [The Problem with Self-Referential Structs](#the-problem-with-self-referential-structs)
      3. [Possible Solutions](#possible-solutions)
      4. [Heap Values](#heap-values)
      5. [`Pin<Box<T>>` and `Unpin`](#pinboxt-and-unpin)
      6. [Stack Pinning and `Pin<&mut T>`](#stack-pinning-and-pinmut-t)
      7. [Pinning and Futures](#pinning-and-futures)
   5. [Executors and Wakers](#executors-and-wakers)
      1. [Executors](#executors)
      2. [Wakers](#wakers)
   6. [Cooperative Multitasking?](#cooperative-multitasking-1)
3. [Implementation](#implementation)
   1. [Task](#task)
   2. [Simple Executor](#simple-executor)
      1. [Dummy Waker](#dummy-waker)
         1. [`RawWaker`](#rawwaker)
         2. [A Dummy `RawWaker`](#a-dummy-rawwaker)
      2. [A `run` Method](#a-run-method)
      3. [Trying It](#trying-it)
   3. [Async Keyboard Input](#async-keyboard-input)
      1. [Scancode Queue](#scancode-queue)
         1. [The `crossbeam` Crate](#the-crossbeam-crate)
         2. [Queue Implementation](#queue-implementation)
      2. [Filling the Queue](#filling-the-queue)
      3. [Scancode Stream](#scancode-stream)
         1. [The `Stream` Trait](#the-stream-trait)
         2. [Implementing `Stream`](#implementing-stream)
      4. [Waker Support](#waker-support)
         1. [AtomicWaker](#atomicwaker)
         2. [Storing a Waker](#storing-a-waker)
         3. [Waking the Stored Waker](#waking-the-stored-waker)
      5. [Keyboard Task](#keyboard-task)
   4. [Executor with Waker Support](#executor-with-waker-support)
      1. [Task Id](#task-id)
      2. [The `Executor` Type](#the-executor-type)
      3. [Spawning Tasks](#spawning-tasks)
      4. [Running Tasks](#running-tasks)
      5. [Waker Design](#waker-design)
         1. [The `Wake` Trait](#the-wake-trait)
         2. [Creating Wakers](#creating-wakers)
      6. [A `run` Method](#a-run-method-1)
      7. [Sleep If Idle](#sleep-if-idle)
...
![](../assets/regain-control-on-interrupt.svg)
...
[_hardware interrupts_]: 07-hardware-interrupts.md
...
![Four states: start, waiting on foo.txt, waiting on bar.txt, end](../assets/async-state-machine-states.svg)
...
![Four states: start, waiting on foo.txt, waiting on bar.txt, end](../assets/async-state-machine-basic.svg)
...
![array at 0x10014 with fields 1, 2, and 3; element at address 0x10020, pointing to the last array element at 0x1001c](../assets/self-referential-struct.svg)
...
![array at 0x10024 with fields 1, 2, and 3; element at address 0x10030, still pointing to 0x1001c, even though the last array element now lives at 0x1002c](../assets/self-referential-struct-moved.svg)
...
[heap allocated]: 10-heap-allocation.md
...
[self-ref-async-await]: 12-async-await.md#self-referential-structs
...
use ph_osx::task::{Task, simple_executor::SimpleExecutor};
...
![QEMU printing "Hello World", "async number: 42", and "It did not crash!"](../assets/qemu-simple-executor.png)
...
[_interrupts_]: 07-hardware-interrupts.md
...
![Scancode queue with 8 slots on the top. Keyboard interrupt handler on the bottom left with a "push scancode" arrow to the left of the queue. Keyboard task on the bottom right with a "pop scancode" queue coming from the right side of the queue.](../assets/scancode-queue.svg)
...
[keyboard interrupt handler]: 07-hardware-interrupts.md#interpreting-the-scancodes
...
use ph_osx::task::executor::Executor; // new
