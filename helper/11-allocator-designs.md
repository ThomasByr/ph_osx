...
2. [Bump Allocator](#bump-allocator)
   ...
   3. [Implementing `GlobalAlloc`](#implementing-globalalloc)
      1. [First Implementation Attempt](#first-implementation-attempt)
      2. [`GlobalAlloc` and Mutability](#globalalloc-and-mutability)
      3. [A `Locked` Wrapper Type](#a-locked-wrapper-type)
      4. [Implementation for `Locked<BumpAllocator>`](#implementation-for-lockedbumpallocator)
      5. [Address Alignment](#address-alignment)
   4. [Using It](#using-it)
   5. [Discussion](#discussion)
      1. [The Drawback of a Bump Allocator](#the-drawback-of-a-bump-allocator)
      2. [Fixing the Test?](#fixing-the-test)
      3. [Reusing All Freed Memory?](#reusing-all-freed-memory)
3. [Linked List Allocator](#linked-list-allocator)
   1. [Implementation](#implementation-1)
      1. [The Allocator Type](#the-allocator-type)
      2. [The `add_free_region` Method](#the-add_free_region-method)
      3. [The `find_region` Method](#the-find_region-method)
         1. [The `alloc_from_region` Function](#the-alloc_from_region-function)
      4. [Implementing `GlobalAlloc`](#implementing-globalalloc-1)
      5. [Layout Adjustments](#layout-adjustments)
   2. [Using it](#using-it-1)
   3. [Discussion](#discussion-1)
      1. [Merging Freed Blocks](#merging-freed-blocks)
      2. [Performance](#performance)
4. [Fixed-Size Block Allocator](#fixed-size-block-allocator)
   1. [Introduction](#introduction-1)
      1. [Block Sizes and Wasted Memory](#block-sizes-and-wasted-memory)
      2. [Deallocation](#deallocation)
      3. [Fallback Allocator](#fallback-allocator)
      4. [Creating new Blocks](#creating-new-blocks)
   2. [Implementation](#implementation-2)
      1. [List Node](#list-node)
      2. [Block Sizes](#block-sizes)
      3. [The Allocator Type](#the-allocator-type-1)
      4. [Calculating the List Index](#calculating-the-list-index)
      5. [Implementing `GlobalAlloc`](#implementing-globalalloc-2)
         1. [`alloc`](#alloc)
      6. [`dealloc`](#dealloc)
   3. [Using it](#using-it-2)
   4. [Discussion](#discussion-2)
   5. [Variations](#variations)
      1. [Slab Allocator](#slab-allocator)
      2. [Buddy Allocator](#buddy-allocator)
5. [Summary](#summary)
...
[previous post]: 10-heap-allocation.md
[map-heap]: 10-heap-allocation.md#creating-a-kernel-heap
[use-alloc-crate]: 10-heap-allocation.md#using-an-allocator-crate
...
![The heap memory area at three points in time:
 ...
 3: A third allocation was added right after the second one; the `next pointer points to the end of the third allocation](../assets/bump-allocation.svg)
...
[global-alloc]: 10-heap-allocation.md#the-allocator-interface
...
[global-allocator]: 10-heap-allocation.md#the-global-allocator-attribute
...
[vga-mutex]: 03-vga-text-buffer.md#spinlocks
...
[`heap_allocation` tests]: 10-heap-allocation.md#adding-a-test
...
[heap-intro]: 10-heap-allocation.md#dynamic-memory

![](../assets/allocation-fragmentation.svg)
...
![](../assets/linked-list-allocation.svg)
...
![](../assets/linked-list-allocator-push.svg)
...
![](../assets/linked-list-allocator-remove-region.svg)
...
[`locked` wrapper]: 11-allocator-designs.md#a-locked-wrapper-type
...
![](../assets/linked-list-allocator-fragmentation-on-dealloc.svg)
...
![](../assets/linked-list-allocator-merge-on-dealloc.svg)
...
![](../assets/fixed-size-block-example.svg).
...
[paging]: 08-paging-introduction.md
...
[bump allocator]: 11-allocator-designs.md#bump-allocator
...
[linked list allocator]: 11-allocator-designs.md#linked-list-allocator
...
[fixed-size block allocator]: 11-allocator-designs.md#fixed-size-block-allocator
...
[slab allocation]: 11-allocator-designs.md#slab-allocator
[buddy allocation]: 11-allocator-designs.md#buddy-allocator
