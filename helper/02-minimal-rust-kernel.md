...
1. [The Boot Process](#the-boot-process)
   1. [BIOS Boot](#bios-boot)
      1. [The Multiboot Standard](#the-multiboot-standard)
   2. [UEFI](#uefi)
2. [A Minimal Kernel](#a-minimal-kernel)
   1. [Installing Rust Nightly](#installing-rust-nightly)
   2. [Target Specification](#target-specification)
      1. [Putting it Together](#putting-it-together)
   3. [Building our Kernel](#building-our-kernel)
      1. [The `build-std` Option](#the-build-std-option)
      2. [Memory-Related Intrinsics](#memory-related-intrinsics)
      3. [Set a Default Target](#set-a-default-target)
   4. [Printing to Screen](#printing-to-screen)
3. [Running our Kernel](#running-our-kernel)
   1. [Creating a Bootimage](#creating-a-bootimage)
      1. [How does it work?](#how-does-it-work)
   2. [Booting it in QEMU](#booting-it-in-qemu)
   3. [Real Machine](#real-machine)
   4. [Using `cargo run`](#using-cargo-run)
...
Currently, we only provide BIOS support, but support for UEFI is planned, too. If you'd like to help us with this, check out the [Github issues](https://github.com/ThomasByr/ph_osx/issues/).
...
Because of these drawbacks we decided to not use GRUB or the Multiboot standard. However, we plan to add Multiboot support to our [bootimage] tool, so that it's possible to load your kernel on a GRUB system too.

### UEFI

(We don't provide UEFI support at the moment, but we would love to! If you'd like to help, please tell us in the [Github issues](https://github.com/ThomasByr/ph_osx/issues/).)
...
We also target `x86_64` systems with our kernel, so our target specification will look very similar to the one above. Let's start by creating a `x86_64-ph_osx.json` file (choose any name you like) with the common content:
...
We're writing a kernel, so we'll need to handle interrupts at some point. To do that safely, we have to disable a certain stack pointer optimization called the _“red zone”_, because it would cause stack corruptions otherwise.
...
[previous post]: 01-freestanding-rust-binary.md
...
```
> cargo build --target x86_64-ph_osx.json
...
[build]
target = "x86_64-ph_osx.json"
```

This tells `cargo` to use our `x86_64-ph_osx.json` target when no explicit `--target` argument is passed. This means that we can now build our kernel with a simple `cargo build`. For more information on cargo configuration options, check out the [official documentation][cargo configuration].
...
After executing the command, you should see a bootable disk image named `bootimage-ph_osx.bin` in your `target/x86_64-ph_osx/debug` directory. You can boot it in a virtual machine or copy it to an USB drive to boot it on real hardware. (Note that this is not a CD image, which have a different format, so burning it to a CD doesn't work).
...
```
> qemu-system-x86_64 -drive format=raw,file=target/x86_64-ph_osx/debug/bootimage-ph_osx.bin
warning: TCG doesn't support requested feature: CPUID.01H:ECX.vmx [bit 5]
```

This opens a separate window with that looks like this:

![QEMU showing "Hello World!"](../assets/qemu.png)
