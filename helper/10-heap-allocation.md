...
1. [Local and Static Variables](#local-and-static-variables)
   1. [Local Variables](#local-variables)
   2. [Static Variables](#static-variables)
2. [Dynamic Memory](#dynamic-memory)
   1. [Common Errors](#common-errors)
   2. [Allocations in Rust](#allocations-in-rust)
   3. [Use Cases](#use-cases)
3. [The Allocator Interface](#the-allocator-interface)
   1. [The `GlobalAlloc` Trait](#the-globalalloc-trait)
      1. [Unsafety](#unsafety)
   2. [A `DummyAllocator`](#a-dummyallocator)
   3. [The `#[global_allocator]` Attribute](#the-global_allocator-attribute)
   4. [The `#[alloc_error_handler]` Attribute](#the-alloc_error_handler-attribute)
4. [Creating a Kernel Heap](#creating-a-kernel-heap)
5. [Using an Allocator Crate](#using-an-allocator-crate)
...
![An outer() and an inner(i: usize) function. Both have some local variables. Outer calls inner(1). The call stack contains the following slots: the local variables of outer, then the argument `i = 1`, then the return address, then the local variables of inner.](../assets/call-stack.svg)
...
![The call stack containing only the local variables of outer](../assets/call-stack-return.svg)
...
[load an interrupt descriptor table]: 05-cpu-exceptions.md#loading-the-idt
...
![The same outer/inner example with the difference that inner has a `static Z: [u32; 3] = [1,2,3];` and returns a `&Z[i]` reference](../assets/call-stack-static.svg)
...
[static `writer`]: 03-vga-text-buffer.md#a-global-interface
[exception handlers]: 05-cpu-exceptions.md#implementation
...
![The inner function calls `allocate(size_of([u32; 3]))`, writes `z.write([1,2,3]);`, and returns `(z as *mut u32).offset(i)`. The outer function does `deallocate(y, size_of(u32))` on the returned value `y`.](../assets/call-stack-heap.svg)
...
![The call stack contains the local variables of outer, the heap contains z[0] and z[2], but no longer z[1].](../assets/call-stack-heap-freed.svg)
...
![QEMU printing "panicked at `allocation error: Layout { size_: 4, align_: 4 }, src/lib.rs:89:5"](../assets/qemu-dummy-output.png)
...
[_"introduction to paging"_]: 08-paging-introduction.md
...
[`mapper` api]: 09-paging-implementation.md#using-offsetpagetable
[_"paging implementation"_]: 09-paging-implementation.md
...
[_translation lookaside buffer_]: 08-paging-introduction.md#the-translation-lookaside-buffer
...
fn kernel_main(boot_info: &'static BootInfo) -> ! {
    use ph_osx::allocator; // new import
...
We show the full function for context here. The only new lines are the `ph_osx::allocator` import and the call to `allocator::init_heap` function. In case the `init_heap` function returns an error, we panic using the [`Result::expect`] method since there is currently no sensible way for us to handle this error.
...
](../assets/qemu-alloc-showcase.png)
...
use ph_osx::allocator::HEAP_SIZE;
