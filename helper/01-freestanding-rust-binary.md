...
2. [Disabling the Standard Library](#disabling-the-standard-library)
   1. [The `no_std` Attribute](#the-no_std-attribute)
3. [Panic Implementation](#panic-implementation)
4. [The `eh_personality` Language Item](#the-eh_personality-language-item)
   1. [Disabling Unwinding](#disabling-unwinding)
5. [The `start` attribute](#the-start-attribute)
   1. [Overwriting the Entry Point](#overwriting-the-entry-point)
6. [Linker Errors](#linker-errors)
   1. [Building for a Bare Metal Target](#building-for-a-bare-metal-target)
   2. [Linker Arguments](#linker-arguments)
      1. [Linux](#linux)
      2. [Windows](#windows)
      3. [macOS](#macos)
      4. [Unifying the Build Commands](#unifying-the-build-commands)
      5. [Should You Do This?](#should-you-do-this)
7. [Summary](#summary)
...
The `Cargo.toml` contains the crate configuration, for example the crate name, the author, the [semantic version] number, and dependencies. The `src/main.rs` file contains the root module of our crate and our `main` function. You can compile your crate through `cargo build` and then run the compiled `ph_osx` binary in the `target/debug` subfolder.
...
[next post]: 02-minimal-rust-kernel.md
